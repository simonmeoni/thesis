...
(extraction de concepts \gls{umls}) et de raffiner itérativement les sorties à partir d'un
...
(BioLORD + juge par modèle de langue) produit des gains relatifs marqués sur les
...
dictionnaires : rappel large mais sur-extraction d'un côté, précision étroite de l'autre. La
...
l'inférence directe du grand modèle, grâce au filtrage implicite des erreurs de ce dernier.

Enfin, sur la vie privée (Q4), la thèse apporte un constat lucide : la génération synthétique
\textit{réduit} mais n'\textit{élimine} pas les risques de réidentification. Les attaques par
liaison et de discrimination révèlent des vulnérabilités significatives, bien au-dessus du
hasard. La cause identifiée est la combinaison distinctive de mots-clés \gls{umls} qui encode
une empreinte patient. De manière contre-intuitive, l'alignement par préférence directe amplifie
ce risque par rapport à l'ajustement supervisé seul. La perturbation ciblée des mots-clés
(remplacement par synonymes d'ontologie) réduit substantiellement le risque de liaison pour
une perte d'utilité aval marginale, offrant un compromis déployable en pratique.
