...
Electronic Health Records (\gls{ehr}) store valuable patient-staff interaction data. Recent advancements in proprietary online large language models (\glspl{llm}) have shown promising capabilities in analyzing \gls{ehr} notes. However, transmitting patient information through external \gls{api} to \glspl{llm} like ChatGPT introduces privacy risks, necessitating alternative approaches that conform to hospital practices.
...
We validate our approach through a proof-of-concept scenario using \gls{mimic}-III, assessing the effectiveness of the generated documents through several downstream tasks: a series of \gls{icd}-9 multi-label classifications of varying complexity and a synthetic Named Entity Recognition (\gls{ner}) task. The results demonstrate that synthetic documents preserve privacy and improve performance when real annotated data are sparse.
