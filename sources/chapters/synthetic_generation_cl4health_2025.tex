...
\section{Improvement of the synthetic generation pipeline}
Our synthetic clinical document generation pipeline addresses a critical need: producing high-quality training data while preserving patient information confidentiality. The approach relies on decoder-type generative models (we use Llama2-7B \cite{touvron2023llama} and Mistral-7B \cite{jiang2023mistral} in our experiments) coupled with an iterative reinforcement improvement process, guided by feedback signals from evaluator models that compare generated documents to public and private reference corpora.

The pipeline is structured around five main phases. (1) A preliminary phase of clinical entity keyword extraction using QuickUMLS \cite{soldaini2016quickumls}, an extractor based on a fixed vocabulary of specialized clinical entities derived from UMLS. This fixed-term approach ensures that no direct identification is possible during pseudonymization, as medical concepts are normalized according to a controlled taxonomy. (2) The first training step consists of supervised fine-tuning on a base of previously declassified documents or documents processed with differential privacy algorithms, integrating noise into gradients during training to guarantee confidentiality properties. (3) The second phase generates candidate synthetic documents from the fine-tuned model. (4) These documents are then evaluated by one or more scorer models that quantify their semantic quality and potential utility. (5) Finally, these scores feed an alignment phase through reinforcement learning, using algorithms such as DPO (Direct Preference Optimization) \cite{rafailov2023dpo} or KTO (Kahneman-Tversky Optimization) \cite{ethayarajh2024kto}, enabling process iteration for continuous improvement.

\subsection{Extension of evaluation benchmarks}
...
\subsection{Attempt to improve the feedback loop}

Given the results obtained with our initial pipeline, we identified potential improvement margins in the quality of synthetic document selection. The underlying principle was to improve this selection quality by using more elaborate evaluators than the simple semantic similarity measures (SemScore) initially employed.

Initially, we applied a medical filtering process adapted from KnowledgeSG \cite{knowledgesg2024}, consisting of identifying whether the generated document or example constitutes a candidate whose subject is the medical domain. This filter ensures that synthetic documents are indeed relevant to the targeted clinical context, discarding off-topic or non-medical generations.

Following this medical filtering, we explored advanced LLM-as-judge evaluation techniques \cite{archit2024criticalevaluation,lee2024selfjudge}, an approach where a large language model evaluates the relevance of retained documents via a prompt specifically designed to calculate a quality score. Unlike traditional automatic metrics, this method exploits LLMs' contextual understanding capabilities to provide a more nuanced evaluation of the relevance and coherence of synthetic documents that passed the first medical filter. For this step, we selected a sample of one thousand documents that obtained a score of five according to KnowledgeSG's evaluation system.
...
The combined application of medical filter and LLM-as-judge (upper section of Table~\ref{tab:alpacare_detailed_results}) shows mixed results: DP-SFT + KTO reaches 0.539 and DP-SFT + DPO 0.525. While these more elaborate evaluation techniques yield slightly better results than the SemScore approach alone, the improvement remains limited and does not necessarily justify the additional complexity cost. The most notable gains are observed primarily against Text-davinci-003, which is the smallest of the evaluator models, while improvements remain marginal for other more sophisticated evaluation models. This performance does not surpass KnowledgeSG, which remains the reference method at 0.562, nor even AlpaCare which maintains its position at 0.536. The limited magnitude of these improvements questions the cost-effectiveness of this more complex approach and raises two hypotheses: either the bootstrap phase is insufficient, or the technique itself does not provide enough information to models for this specific domain.

Our exchanges with the authors of KnowledgeSG also revealed that certain figures from the original paper should be taken with caution, identifying potential problems in how results were constructed. Moreover, the use of differential privacy can unfortunately further noise the model, although it guarantees the absence of indistinguishable leaks. The comparison between filtered and unfiltered versions thus suggests that the improvement provided by this combined approach remains moderate, questioning the effectiveness of the bootstrap process or the technique itself for this specific domain.
...
KnowledgeSG & \uline{\textbf{0.776}} & 0.530 & 0.457 & 0.488 & \uline{\textbf{0.562}} \\
...
\subsection{Privacy analysis and information leakage}
...
Our approach fundamentally differs from traditional methodologies such as canary attacks used in KnowledgeSG. This technique proves de facto inapplicable to our context since our method relies on keywords extracted from an ontological base (UMLS) that do not allow direct patient identification. These normalized keywords constitute by nature non-identifying elements, making the canary attack moot. Moreover, our threat model differs: the end user only accesses the synthetic dataset without being able to directly query the generative model.
...
We implemented two complementary types of attacks. The linkage attack (1-vs-N) attempts to globally attribute each synthetic document to its source patient among the entire dataset, with a near-zero random baseline. The discrimination attack (1-vs-2) performs a repeated discrimination test: for each synthetic document, we compare it three times to pairs composed of the real patient's private document versus that of a different patient, with a theoretical baseline of 50\%.
...
These results highlight a fundamental paradox of our approach. The linkage attack, which simulates an adversary attempting to identify a document's origin among all possible patients, presents moderate success rates (45-76\%). In contrast, the discrimination attack, which only requires binary distinction between two candidates, achieves near-perfect performance (89-96\%). This disparity reveals that synthetic documents, while not trivially associable with their source in a global identification context, nevertheless retain characteristic fingerprints sufficiently distinctive to enable quasi-certain attribution in direct comparisons. This observation raises questions about the actual effectiveness of our privacy preservation method and suggests the necessity of developing more robust generation techniques to mitigate these vulnerabilities in terms of re-identification.
...
\begin{figure}[h]
    \centering
    \resizebox{\columnwidth}{!}{\input{figures/synthetic_generation_cl4health_2025/labels.pgf}}
