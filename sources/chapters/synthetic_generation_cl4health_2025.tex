...
In recent years, Proprietary Online Large Language Models (\glspl{llm}) such as ChatGPT have shown impressive results using zero or few-shot techniques in analyzing these notes~\cite{agrawal_large_2022, meoni2023large, hu_improving_2024}.
...
Many recent studies focus on creating synthetic data, particularly for generating clinical data. For instance,~\citet{kweon_publicly_2023} proposes to train \glspl{llm} for different purposes using synthetic clinical data generated by online \glspl{llm}. \citet{xie_differentially_2024} has developed AUG-PE, a high-quality differential privacy synthetic text generation method leveraging \gls{api} access.
...
To assess the performance of \gls{llm} in Multiple Questions Choices in the medical field, \citet{griot_multiple_2024} developed a fictional medical benchmark to isolate the knowledge of the \gls{llm} from its test-taking abilities. \citet{liTwoDirectionsClinical2023} generated a synthetic dataset of Alzheimer's Disease relative signs. As this task is relatively complex, \gls{llm} created the dataset by incorporating expert knowledge taxonomy. Finally, the \citet{hiebelCanSyntheticText2023, xie_differentially_2024} works focus on generating a synthetic dataset of clinical cases for the \gls{ner} task to study the effectiveness of real clinical data versus synthetic data.

\paragraph{Self-Rewarding:}
Reinforced Self-Training is an offline \gls{rl} algorithm proposed by~\citet{gulcehre_reinforced_2023} for self-align \glspl{llm} generating a dataset from the initial \gls{llm} policy and using it to improve the policy via offline \gls{rl}. Instruction back translation \cite{li_self-alignment_2023} is a scalable method that automatically labels human-written text with corresponding instructions by finetuning a \gls{lm} on a small seed dataset and a web corpus to generate and selecting high-quality examples for further finetuning. \citet{yuan_self-rewarding_2024} use the trained \gls{llm} to provide rewards via \gls{llm}-as-a-Judge prompting, improving both instruction following and reward provision. \citet{lee2023rlaif} introduces Reinforcement Learning from \gls{ai} Feedback (\gls{rlaif}) as an alternative, using an off-the-shelf \gls{llm} to generate preference labels. \gls{rlaif} achieves comparable or superior performance to \gls{rlhf} in many tasks, such as those rated by humans.
